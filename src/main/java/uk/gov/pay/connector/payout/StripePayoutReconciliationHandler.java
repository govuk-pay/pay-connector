...
public class StripePayoutReconciliationHandler implements PayoutReconcileHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(StripePayoutReconciliationHandler.class);
    ...
    @Inject
    public StripePayoutReconciliationHandler(StripeSdkClient stripeClient,
    ...
    public boolean reconcile(PayoutReconcileMessage payoutReconcileMessage) {
        try {
            ...
            throw new RuntimeException("error retrieving Stripe balance transactions for payout reconciliation: " + e.getMessage(), e);
        } finally {
